...
export class TFLiteModel {
    ...
    predict(inputs, config) {
        ...
        for (let i = 0; i < modelOutputs.length; i++) {
            ...
            const outputTensor = tensor(modelOutput.data(), this.getShapeFromTFLiteTensorInfo(modelOutput));
 ...
 * @doc {heading: 'Models', subheading: 'TFLiteModel'}
 */
export async function loadTFLiteModel(modelUrl, options = DEFAULT_TFLITE_MODEL_RUNNER_OPTIONS) {
    // Handle tfhub links.
    ...
    const tfliteModelRunner = await tfliteWebAPIClient.tfweb.TFLiteWebModelRunner.create(modelUrl, options);
    return new TFLiteModel(tfliteModelRunner);
}
export async function loadTFLiteModelFromBuffer(buffer, options = DEFAULT_TFLITE_MODEL_RUNNER_OPTIONS) {
    const tfliteModelRunner = await tfliteWebAPIClient.tfweb.TFLiteWebModelRunner.createFromBuffer(buffer, options);
    return new TFLiteModel(tfliteModelRunner);
}
