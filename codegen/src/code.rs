...
fn rune_custom_section(rune: &Rune) -> Result<(usize, Literal), Error> {
    let jsonified = serde_json::to_string(rune)
...
fn preamble(rune: &Rune) -> Result<TokenStream, Error> {
    let (len, data) = rune_custom_section(rune)?;
    ...
    Ok(quote! {
        ...
        #[link_section = "rune_graph"]
        static RUNE_GRAPH: [u8; #len] = *#data;
