...
def prepare_tf_micro_spectrogram_computer():
    ...
    with tf.compat.v1.get_default_graph().name_scope('data'):
        wav_signal_placeholder = tf.compat.v1.placeholder(tf.int16, shape=16000, name='wav_signal_placeholder')
        micro_frontend = frontend_op.audio_microfrontend(wav_signal_placeholder, sample_rate=sample_rate, 
                                                         window_size=window_size_ms,
            window_step=window_step_ms, num_channels=num_channels, upper_band_limit=upper_band_limit, 
            lower_band_limit=lower_band_limit, min_signal_remaining=min_signal_remaining, smoothing_bits=smoothing_bits, 
            even_smoothing=even_smoothing, odd_smoothing=odd_smoothing, enable_pcan=enable_pcan, pcan_strength=pcan_strength, 
            pcan_offset=pcan_offset, gain_bits=gain_bits, enable_log=enable_log, scale_shift=scale_shift, out_scale=1,
            out_type=tf.float32)
...
class AudioProcessor(object):
    ...
    def precompute_rust_spectrograms(self):
        ...
        for fold in ["training", "validation", "testing"]:
            for sample in self.data_index[fold]:
                # print(sample["label"], sample["file"])
                if sample["label"] == "_silence_":
                    signal = np.random.randint(-32768, 32767, [1, 16000])
                else:
    ...
    def precompute_tf_spectrograms(self):
        self.specs = {"training": [], "validation": [], "testing": []}
        
        wav_signal_placeholder, output_ = prepare_tf_micro_spectrogram_computer()

        for fold in ["training", "validation", "testing"]:
            for sample in self.data_index[fold]:
                # print(sample["label"], sample["file"])
                if sample["label"] == "_silence_":
                    signal = np.random.randint(-32768, 32767, 16000)
                else:
                    sample_rate, signal = wavfile.read(sample["file"])
                
                signal = signal.astype(np.float32)
                voice_signal_normalised = (signal-np.min(signal))/(np.max(signal)-np.min(signal))
                voice_signal_int16 = (voice_signal_normalised*32768).astype(np.int16)
                
                if len(voice_signal_int16) > 16000:
                    voice_signal_int16 = voice_signal_int16[:16000]
                elif len(voice_signal_int16) < 16000:
                    to_add_int16 = np.zeros(16000-len(voice_signal_int16), dtype=np.int16)
                    voice_signal_int16 = np.hstack([voice_signal_int16, to_add_int16])

                input_dict = {wav_signal_placeholder: voice_signal_int16}
                with tf.compat.v1.Session() as sess:
                    [spec] = sess.run([output_], feed_dict=input_dict)
                
                self.specs[fold].append(
                    {"label": sample["label"], "spec": spec.flatten()})
    ...
    def prepare_processing_graph(self, model_settings, summaries_dir):
        ...
        with tf.compat.v1.get_default_graph().name_scope('data'):
            ...
            if model_settings['preprocess'] == 'rune': # Plug in pre-computed spectrogram
                self.spectrogram_placeholder = tf.compat.v1.placeholder(
                    tf.int8, [1, 1960], name='input_spectrogram')
                self.output_ = self.spectrogram_placeholder

            else: # Compute spectrograms online in the graph
                ...
                if model_settings['preprocess'] == 'average':
                    self.output_ = tf.nn.pool(
                        ...
                        window_shape=[
                            1, model_settings['average_window_width']],
    ...
    def get_data_precomputed(self, how_many, offset, mode):
        """ Retrieve precomputed spectrograms and their ground truth labels """
        
        data = []
        labels = []
        if how_many < len(self.specs[mode]):
            selected_samples = np.random.choice(self.specs[mode], size=how_many, replace=False)
        else:
            selected_samples = self.specs[mode][offset: offset+how_many]
            
        for sample in selected_samples:
            labels.append(self.word_to_index[sample["label"]])
            data.append(sample["spec"])
        
        return np.vstack(data).astype(np.float32), np.array(labels).astype(np.float32)
